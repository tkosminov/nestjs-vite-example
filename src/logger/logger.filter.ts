...
function filterUnknown(keywords: string[], value: unknown, field?: string): unknown {
  if (field && keywords.includes(field)) {
    return FILTERED;
  }
  ...
  if (value) {
    if (Array.isArray(value)) {
      ...
      return filterObject(keywords, value as Record<string, unknown>);
    } else {
...
function filterObject(keywords: string[], obj: Record<string, unknown>) {
