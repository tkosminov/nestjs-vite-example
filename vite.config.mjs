...
const repl_nest_custom = resolve(src_path, 'repl-custom/repl.ts');
const cli_path = resolve(src_path, 'typeorm/typeorm.cli.ts');

const app_path = process.env.RUN_BUILD !== 'true' ? main_path : [main_path, repl_nest_path, repl_nest_custom, cli_path];
